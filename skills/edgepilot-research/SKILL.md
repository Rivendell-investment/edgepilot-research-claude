...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/edgepilot-research/scripts/install_runtime.py"
```

```bat
py -3 "${CLAUDE_PLUGIN_ROOT}/skills/edgepilot-research/scripts/install_runtime.py"
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/edgepilot-research/scripts/install_runtime.py" --wheelhouse /path/to/wheels
```
...
python3 "${CLAUDE_PLUGIN_ROOT}/skills/edgepilot-research/scripts/install_runtime.py" --wheelhouse "$PWD"
```

```bat
curl.exe -L --fail -A "Mozilla/5.0" -o "%LOCKED_FILENAME%" "%LOCKED_NAUTILUS_URL%"
py -3 "${CLAUDE_PLUGIN_ROOT}/skills/edgepilot-research/scripts/install_runtime.py" --wheelhouse %CD%
```
...
This Skill is distributed for Claude Code. Do not claim that runtime downloads, native installation, or local process execution work on another host unless that exact host has passed its release checklist. If local execution is unavailable, explain the limitation; do not silently switch to the Live product, PyPI, arbitrary code execution, or a remote trading service.
